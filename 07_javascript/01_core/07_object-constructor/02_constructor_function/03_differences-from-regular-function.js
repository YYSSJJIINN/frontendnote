...
console.log(this);
/* Note. new 연산자를 사용하지 않고 일반 함수로서 호출될 때, 여기서 this는 전역 객체(global object)를 가리킴.
전역 객체에 대한 내용은 11_standard--built-in-object에서 추가 설명한다.
*/
...
function Dog(name, age) {
    ...
    // 이 함수가 new 연산자와 함께 호출되지 않았다면 new.target은 undefined(Falsy값)이다.
    if(!new.target) {   //Falsy 값을 !로 거꾸로 하면 Truthy 값이 된다.
